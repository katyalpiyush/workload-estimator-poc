...
func EstimateResources(request models.ComputeRequest) models.ComputeResponse {
	...
	for _, group := range request.ServiceGroups {
		...
		for _, service := range group.Services {
			...
			case "index":
				ram, cpu, disk, diskIO = services.EstimateResourcesForIndex(request.Dataset, request.Workload, int(nodes))
			case "query":
			...
			case "eventing":
				ram, cpu, disk, diskIO = services.EstimateResourcesForEventing(request.Dataset, request.Workload, int(nodes))
			case "analytics":
